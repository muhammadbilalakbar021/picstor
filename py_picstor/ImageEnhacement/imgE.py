...
def maxEntropyEnhance(I, isBad, a=-0.3293, b=1.1258):
    ...
    isBad = isBad * 1
    isBad = resize(isBad, output_shape=(50, 50))
    # isBad = cv2.resize(isBad, (50,50))
    isBad[isBad < 0.5] = 0
    isBad[isBad >= 0.5] = 1
    Y = Y[isBad == 1]
...
def Ying_2017_CAIP(img, mu=0.5, a=-0.3293, b=1.1258):
    ...
    t_b = np.max(I, axis=2)
    # t_our = resize(t_b, output_shape=(64, 64)).reshape((1, 64 * 64 * 3)).T
    t_our = cv2.resize(t_b, (t_b.shape[1], t_b.shape[0]), interpolation=cv2.INTER_AREA)
...
def EahanceImage(image):
    img = cv2.imread(r'../src/assets/images/'+image)
    path = r'pics/'
